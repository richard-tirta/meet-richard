...
services themselves — colleagues did, and I consumed them.
...
gallery and a **family business** — marketing site through back-office
operations, all in production with real daily users. Being family is how I
ended up as its only engineer, and why I own every architectural decision
in it.
...
- **Admin platform**: a full Outlook-integrated operations app (Microsoft
  Graph) built on the signed-in staff member's own mailbox rather than a
  shared one — mail triage with **LLM-powered classification** into a
  six-bucket taxonomy, consignment intake with a remote tokenized e-sign
  agreement flow, label printing with layouts that adapt to the data each
  item has, inventory projections from the legacy POS, buyer routing, and a
  web-attribution loop closing ad spend back to accepted consignments.
