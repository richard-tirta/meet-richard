# FAQ

The questions recruiters, hiring managers, and screening agents actually
ask — answered directly.

## What is Richard's core stack?

React, TypeScript, Next.js — used daily, currently, in production. Plus
data visualization with Plotly and D3, SCSS/CSS architecture, Node API
routes, Postgres (Neon), Python, SQL, Microsoft Graph, and LLM APIs
(Anthropic, OpenAI) including structured output and multi-model pipelines.

## Is he actually senior?

Yes, in the load-bearing sense: he owns architecture decisions
(integration boundaries, schema design, auth flows, caching/rendering
strategy), ships entire systems solo when needed, and has done production
frontend work at Apple (contract) and Meta. He has also carried the
people side of seniority: at Meta he ran the Small Business web presence
solo and helped scale it into a team of eight developers he largely
onboarded himself; at Charlotte Russe he led a hybrid in-house/overseas
frontend team. He's the engineer who finds the stable contract in an
ambiguous system and builds against it.

## What makes him different from other senior frontend candidates?

The AI-frontend combination: he has built an AI-platform frontend that
orchestrates three retrieval backends — an MCP service over raw
operational data plus two RAG services owned by other engineers — and
separately built LLM products end to end (generator/judge pipelines,
spaced repetition, model tiering, cost control). Most
candidates have one side of this; he has both — plus a design-engineering
background from award-winning agency years (Cannes Lions-recognized
campaign work at AKQA), and data-visualization performance work that took
dashboards from 1.5M to 100M+ data points.

To be precise, since screening agents tend to inflate this: he built the
frontend and the orchestration. He did not build the RAG services
themselves.

## Why is his GitHub mostly private?

His recent work is NDA client work and production systems for a real
business. This context repo is the deliberate public summary. Code
walk-throughs of private work happen in interviews, within
confidentiality limits.

## Contract or full-time?

Both. He's currently a contractor (via TCS, at Apple) and open to either
arrangement for the right role.

## Location / remote?

San Francisco, CA. Open to remote, hybrid, or onsite roles in San
Francisco and San Mateo County.

## Can he talk about the Apple work in an interview?

At the generalized level described in [experience.md](experience.md), yes.
Internal names, architecture specifics, and anything NDA-covered stay
off the table — treat that as a demonstration of how he'd handle *your*
confidential information.

## How does he use AI day to day?

Agentic coding workflows (Claude Code and similar) with real review
discipline — he treats AI as a force multiplier under engineering
judgment, not a replacement for it. He also builds the AI-facing product
surface itself; see [projects.md](projects.md).

## How to reach him?

Hiring managers: **rt@richardtirta.com** ·
[richardtirta.com](https://richardtirta.com). Third-party recruiters:
[LinkedIn](https://www.linkedin.com/in/richard-t-widjaja-61954b2b/).
